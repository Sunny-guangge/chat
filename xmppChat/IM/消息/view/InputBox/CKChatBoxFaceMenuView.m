...
@interface CKChatBoxFaceMenuView ()
...
@implementation CKChatBoxFaceMenuView
...
- (void)setFaceGroupArray:(NSMutableArray *)faceGroupArray
{
    ...
    for (CKFaceGroup *group in faceGroupArray) {
        ...
        [button setTag:i ++];
        [button addTarget:self action:@selector(didClickGroupImage:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [self didClickGroupImage:[self.faceMenuViewArray firstObject]];
}
...
- (void)didClickGroupImage:(UIButton *)button
{
    for (UIButton *button in self.faceMenuViewArray) {
        [button setBackgroundColor:[UIColor whiteColor]];
    }
    
    [button setBackgroundColor:DEFAULT_CHATBOX_COLOR];
    
    if ([[_faceGroupArray objectAtIndex:button.tag] faceType] == CKFaceTypeEmoji) {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(chatBoxFaceMenuView:didSelectedFaceMenuIndex:)]) {
        [_delegate chatBoxFaceMenuView:self didSelectedFaceMenuIndex:button.tag];
    }
}
...
- (UIButton *)addButton
{
    if (_addButton == nil) {
        ...
        [_addButton addTarget:self action:@selector(didClickAddButton:) forControlEvents:UIControlEventTouchDown];;
    }
    return _addButton;
}
...
- (NSMutableArray *)faceMenuViewArray
{
    if (_faceMenuViewArray == nil) {
        _faceMenuViewArray = [NSMutableArray array];
    }
